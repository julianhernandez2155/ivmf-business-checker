...
def check_business(
    ...
    prompt = f"""You are a research analyst verifying whether a specific business is still actively operating. Think like a thorough investigator — not just someone running a quick search.

Business name: "{name}"
Location on file: {location}
{website_note}{scrape_section}
...
FAST PATH — use this if the website content above shows a real, currently operating business. Ask yourself: does this look like a live business, or does it look abandoned?
...
Run all of these queries:
  • "{name}" {location}
  • "{name}" [relevant industry term — e.g., "catering", "sauce", "jerky", "consulting"]
...
Search Google Maps for "{name}" near {location}.
...
CHANNEL 3 — FACEBOOK
Search Facebook for a business page named "{name}".
...
CHANNEL 5 — LINKEDIN
Search LinkedIn for the company "{name}" and for its owner/founder.
...
Search Amazon, Etsy, specialty food retailers, or other relevant platforms for "{name}" products.
...
Search BBB (bbb.org), Yelp, industry-specific directories, or trade association member lists for "{name}".
...
CONFIDENCE SCORE RUBRIC (use this — do not assign arbitrary numbers):
  ...
  Uncertain 60–80: Found the business but last confirmed activity was 2020–2022 or signals conflict.
  Uncertain 40–59: Very thin evidence; genuinely cannot determine active or closed.
...
Respond with the status, confidence score, and a one-sentence evidence summary naming the specific source and its recency (e.g., "website dead, Google Maps permanently closed January 2025, Facebook inactive since 2020" or "website loads with active e-commerce, products in cart as of 2026")."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            cost      = _calculate_cost(data.get("usage", {}))
            ...
            return _error_result(f"Parse error: {str(e)[:120]}")

        except requests.exceptions.Timeout:
            if attempt < max_retries - 1:
                time.sleep(10)
                continue
            return _error_result("Request timed out after max retries.")

        except requests.exceptions.RequestException as e:
            return _error_result(f"Request error: {str(e)[:120]}")

        except Exception as e:
            return _error_result(f"Unexpected error: {str(e)[:120]}")

    return _error_result("Failed after max retries.")
