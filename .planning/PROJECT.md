...
| Routing labels stored as decomposed fields, not display strings (Phase 2) | Per Codex review 2026-05-15: storing `"Likely Closed - outreach recommended"` as a column value couples persistence to UI copy. Store `status` code + `routing_action` code + `routing_reason` + `contact_available` bool + `deadline_at` timestamp; the 6-value display label is a derived view. RoutingLabel enum in `business_checker/eval/routing_labels.py` stays as the eval-CI distribution buckets, but `verifications.routing_label` is NOT added as a column in Phase 2 — instead Phase 2 adds the component columns. | — Pending (Phase 2) |
| Pre-call claim pattern for API idempotency (Phase 2) | Per Codex review 2026-05-15: the `UNIQUE (provider, request_hash)` on `api_calls` prevents double-row writes but NOT double API calls. Phase 2 worker must INSERT `api_calls(status='in_progress', provider, request_hash) ON CONFLICT DO NOTHING` BEFORE calling Perplexity/FireCrawl. Only the winner row's worker makes the external call; the loser row's worker reads the cached result once the winner completes. Closes Pitfall P1. | — Pending (Phase 2) |
